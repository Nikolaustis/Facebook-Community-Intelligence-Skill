...
if ($Task -eq "phase2" -and -not $DirectBackground) {
  ...
  function Register-AndStartPhase2Task([string]$LauncherMode) {
    ...
    Register-ScheduledTask -TaskName $ScheduledTaskName -Action $action -Trigger $triggers -Settings $settings -Principal $principal -Description "Facebook Group Monitor V7.1.0 Phase 1.5 + Phase 2; pre-browser name relevance filtering; safe alias boundaries; same-business-region preservation; isolated supervisor logs; verified startup; reboot-resumable; self-deletes after execution." -Force | Out-Null
