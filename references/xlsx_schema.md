...
`group_name` 必须是群组的可见名称，不得包含 Facebook 本地化界面的头像或个人资料图片包装文本。群名清洗应在语言判断和地区判断之前完成。

Phase 1/Phase 2 的原始名称、名称来源和清洗原因属于 JSON 审计字段，不强制插入既有 `detail` 公共列，以免破坏历史字段顺序。已经生成的工作簿可通过 `repair_avatar_name_pollution_xlsx.js` 生成新的修复副本和审计工作表。
